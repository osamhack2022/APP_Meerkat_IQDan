# 개발 규칙
## 칸반 규칙 (할일 정리)
ClickUp 칸반을 14일 Free-trial로 사용중. 이후에는 해커톤 종료일까지 팀장이 사비로 구매할 예정.
#### 왜 칸반을 쓰는지
누가 무슨일을 하고 있고 앞으로 무슨일을 해야되는지 실시간을 관리하기 위함임. 그리고 커밋 관리도 편해짐.
#### 티켓이란?
나중에 commit 기록 검색하는것 보다 티켓으로 커밋 기록 관리하는게 편함. ClickUp 칸반에 "mk-숫자"로 되어있는 형식에 따라 커밋을 똑같이 작성하면
나중에 특정 기능 관련 커밋 검색할 때 칸반에서 검색하면됨. 자기가 뭔가 할일이 생각날 때 마다 미완료 티켓에 티켓 추가하고 진행중인 것은 진행중인 티켓으로
옮긴 후 자신에게 assign하면됨. 태그 잘 달아야 나중에 검색 편함.
