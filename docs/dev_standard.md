...
- 나중에 commit 기록 검색하는것 보다 티켓으로 커밋 기록 관리하는게 편합니다. ClickUp에 현재 자동으로 할일 마다 티켓 번호가 매겨지기 때문에 Custom Ticket ID를 그대로 쓰면됩니다. ClickUp 칸반에 "MK-숫자"로 되어있는 형식에 따라 커밋을 똑같이 작성하면 나중에 특정 기능 관련 커밋 검색할 때 칸반에서 검색하면됩니다. (i.e. git commit -m "MK-22 fix: fix something") 
